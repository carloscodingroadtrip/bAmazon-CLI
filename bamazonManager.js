			...
			connection.query('SELECT * FROM bamazondb.products', function(error, results, fields) {
				...
				switch (answers.menu) {
					...
					case 'View Low Inventory':
						connection.query('SELECT * FROM bamazondb.products WHERE stock_quantity<5;', function(
							error,
							results,
							fields
						) {
							...
							results.forEach(value => {
								table.push([
									...
									value.department_name,
									chalk.green(value.price),
									chalk.red(value.stock_quantity),
								]);
							});
							console.log(table.toString());
							createTable();
...
function increaseInventory() {
	inquirer
		...
		.then(answers => {
			...
			let exists = false;
			connection.query('SELECT * FROM bamazondb.products WHERE item_id=?;', [stockID], function(
				error,
				results,
				fields
			) {
			...
			connection.query(
				'UPDATE bamazondb.products SET stock_quantity=stock_quantity + ? WHERE item_id=?;',
				[stockQty, stockID],
...
function addNewProduct() {
	inquirer
		...
		.then(answers => {
			connection.query(
				'INSERT INTO bamazondb.products (product_name, product_description, department_name, price, stock_quantity, product_sales) VALUES (?, ?, ?, ?, ?, 0);',
				[answers.name, answers.description, answers.department, answers.price, answers.stock],
				function(error, results, fields) {
					if (error) throw error;
...
function removeProduct() {
	inquirer
		...
		.then(answers => {
			...
			let exists = false;
			connection.query('SELECT * FROM bamazondb.products WHERE item_id=?;', [productID], function(
				error,
				results,
				fields
			) {
			...
			connection.query('DELETE FROM bamazondb.products WHERE item_id=?;', [productID], function(
				error,
				results,
				fields
			) {
