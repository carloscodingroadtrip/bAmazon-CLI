		...
		connection.query('SELECT * FROM bamazondb.products', function(error, results, fields) {
			...
			inquirer
				...
				.then(answers => {
					...
					let id = idArr[0];
					connection.query('SELECT * FROM bamazondb.products WHERE item_id = ?;', [id], function(
						error,
						results,
						fields
					) {
						if (results[0].stock_quantity >= answers.numItems) {
							console.log(
								chalk.blue(answers.product_choice),
								chalk.yellow(` - quantity ${answers.numItems} has been added to your cart.`)
							);
							...
							connection.query(
								`UPDATE bamazondb.products SET stock_quantity = ${results[0].stock_quantity -
									answers.numItems}, product_sales = ${results[0]
									.product_sales} + ${answers.numItems} * ${results[0].price} WHERE item_id = ${id}`,
							...
							console.log(
								chalk.red(
									'Sorry, current stock is insufficient to complete your order. Please select another item or number of items.'
								)
							);
							shop();
						}
